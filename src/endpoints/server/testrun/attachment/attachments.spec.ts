...
describe(path.relative(process.cwd(), import.meta.filename), () => {
  ...
  describe("addEvidence", () => {
    for (const [version, endpoint] of [
      ["v1", XRAY_CLIENT_SERVER.testRuns.evidence.addEvidence.v1],
      ["v2", XRAY_CLIENT_SERVER.testRuns.evidence.addEvidence],
    ] as const) {
      describe(version, () => {
        beforeEach(async () => {
      ...
      it("adds evidence data", async () => {
        await endpoint(DATA_SERVER.testExecutions.addingAttachments.tests[0].testRunId, {
  ...
  describe("deleteEvidenceByName", () => {
    for (const [version, endpoint] of [
      ["v1", XRAY_CLIENT_SERVER.testRuns.evidence.deleteEvidenceByName.v1],
      ["v2", XRAY_CLIENT_SERVER.testRuns.evidence.deleteEvidenceByName],
    ] as const) {
      describe(version, () => {
  ...
  describe("deleteEvidenceById", () => {
    for (const [version, endpoint] of [
      ["v1", XRAY_CLIENT_SERVER.testRuns.evidence.deleteEvidenceById.v1],
      ["v2", XRAY_CLIENT_SERVER.testRuns.evidence.deleteEvidenceById],
    ] as const) {
      describe(version, () => {
        it("deletes evidence data", async () => {
          const content = await XRAY_CLIENT_SERVER.testRuns.evidence.getEvidence(
